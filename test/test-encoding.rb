...
class EncodingTest < Test::Unit::TestCase
  def test_1
    ...
    returned = JWT::Multisig.encode(payload, private_keychain.slice(*signers), algorithms.slice(*signers))
  ...
  def test_3
    ...
    assert_raises JWT::EncodeError do
      JWT::Multisig.encode({}, private_keychain.slice(*signers), algorithms.slice(signers.sample))
    end
  end
end
